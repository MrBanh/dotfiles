// Local copy of the tmux-agent-sidebar OpenCode plugin.
//
// This lives in ~/.config/tmux/scripts (outside the plugin's git repo) so
// plugin updates cannot clobber local customizations. OpenCode loads it via a
// symlink in ~/.config/opencode/plugins/. The upstream copy at
// <repo>/.opencode/plugins/tmux-agent-sidebar.js is left pristine and unused.
//
// Local change vs. upstream: fire `session-start` at plugin load (TUI startup)
// so an idle OpenCode pane shows in the sidebar before the first prompt — see
// the TmuxAgentSidebar factory below.
...
// Because this file lives outside the plugin repo, we can't walk up from
// import.meta.url to find hook.sh. Resolve it from known install locations
// instead, falling back to a `tmux-agent-sidebar` binary on $PATH.
const HOOK_CANDIDATES = [
  join(homedir(), ".config/tmux/plugins/tmux-agent-sidebar/hook.sh"),
  join(homedir(), ".tmux/plugins/tmux-agent-sidebar/hook.sh"),
];

const HOOK_COMMAND = (() => {
  const hookScript = HOOK_CANDIDATES.find((candidate) => existsSync(candidate));
  return hookScript
